...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const stockProducts = await api.get(`/stock/${productId}`)
      const newCart = [...cart];
      const productAlreadyExists = newCart.find(product => product.id === productId)

      const stock = stockProducts.data.amount
      const amountProduct = productAlreadyExists ? productAlreadyExists.amount : 0;
